...
>
  ...
  >
    ...
    <p className="text-base sm:text-lg md:text-2xl text-gray-300 max-w-xl">
      We’re building innovative IT solutions. Explore opportunities to grow with us!
    </p>
  </motion.div>
          ...
          >
            {careersData.jobs.map((job) => (
              <motion.div
                ...
                variants={jobCard}
                whileHover="hover"
                className="p-8 bg-black border border-gray-700 rounded-xl cursor-pointer"
              >
                <h3 className="text-2xl font-semibold mb-2">{job.title}</h3>
                <p className="text-gray-400 text-lg mb-1">{job.location}</p>
                <p className="text-gray-500 font-medium mb-4">{job.type}</p>
                <p className="text-gray-300 mb-6">{job.description}</p>
                <button
                  onClick={() => navigate("/company/careers/apply")}
                  className="px-6 py-3 bg-white text-black font-semibold rounded-xl hover:bg-gray-300 transition-all"
                >
                  Apply Now
                </button>
              </motion.div>
